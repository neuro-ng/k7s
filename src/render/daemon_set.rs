...
impl Renderer for DaemonSetRenderer {
    ...
    fn render(&self, obj: &Value) -> RenderedRow {
        ...
        let desired   = obj.pointer("/status/desiredNumberScheduled").and_then(|v| v.as_i64()).unwrap_or(0);
        let current   = obj.pointer("/status/currentNumberScheduled").and_then(|v| v.as_i64()).unwrap_or(0);
        let ready     = obj.pointer("/status/numberReady").and_then(|v| v.as_i64()).unwrap_or(0);
        let updated   = obj.pointer("/status/updatedNumberScheduled").and_then(|v| v.as_i64()).unwrap_or(0);
        let available = obj.pointer("/status/numberAvailable").and_then(|v| v.as_i64()).unwrap_or(0);
